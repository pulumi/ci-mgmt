...
### Mise version ownership

The canonical generated Mise defaults are
[`provider-ci/internal/pkg/templates/all/.config/mise.toml`](./provider-ci/internal/pkg/templates/all/.config/mise.toml)
and
[`provider-ci/internal/pkg/templates/all/.config/mise.test.toml`](./provider-ci/internal/pkg/templates/all/.config/mise.test.toml).
The `all` template installs them for bridged and native providers. Renovate
updates these canonical TOML templates in ci-mgmt, and its `make build`
post-upgrade task regenerates the committed `provider-ci/test-providers`
snapshots. Renovate must not extract dependencies directly from those generated
snapshots.

The generated `.config/mise*.toml` files in provider repositories are also
owned by ci-mgmt and must not be updated independently by downstream Renovate.
Provider-owned root `mise.toml` files are deliberately separate: providers may
use them to override or augment the generated defaults, and Renovate should
continue to manage those root files.

The downstream exclusion belongs in a provider-scoped preset in
[`pulumi/renovate-config`](https://github.com/pulumi/renovate-config), rather
than in generated provider `renovate.json5` files. Generating or replacing those
files would overwrite provider-specific Renovate rules. The provider preset
should include the following rule, and provider repositories should adopt that
preset while preserving their existing local configuration:

```json5
{
  packageRules: [
    {
      description: "Mise configuration generated by pulumi/ci-mgmt",
      matchManagers: ["mise"],
      matchFileNames: ["/^\\.config\\/mise(?:\\.[^/]+)?\\.toml$/"],
      enabled: false,
    },
  ],
}
...
The anchored path intentionally excludes root `mise.toml`. This downstream
preset rollout is a dependency of the complete ownership change; the local
ci-mgmt Renovate configuration only ignores generated test-provider snapshots.
