...
[env]
_.pulumi-versions = { module_path = "#{{ .Config.ModulePath }}#" }
...
func (migrateCimgmtToMise) Migrate(templateName, outDir string) error {
	...
	if len(strings.TrimSpace(string(mise.content))) == 0 {
		...
		// Migrate existing files from _.source to _.pulumi-versions
		updated := migrateSourceToEnvPlugin(string(mise.content))
		if updated != string(mise.content) {
			mise.content = []byte(updated)
			needsWrite = true
		}
	}
...
// migrateSourceToEnvPlugin replaces the old _.source directive with the new _.pulumi-versions env plugin
func migrateSourceToEnvPlugin(content string) string {
	// Replace the old _.source line with the new env plugin syntax
	// This handles the migration from bash script to Windows-compatible Lua env plugin
	oldPattern := `_.source = "{{config_root}}/scripts/get-versions.sh"`
	newPattern := `_.pulumi-versions = { module_path = "#{{ .Config.ModulePath }}#" }`

	return strings.ReplaceAll(content, oldPattern, newPattern)
}
