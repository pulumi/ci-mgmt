...
bin/%/$(PROVIDER) bin/%/$(PROVIDER).exe: bin/jsign-7.0.jar
	...
	if [[ "${GOOS}" = "windows" && "${SKIP_SIGNING}" != "true" ]]; then \
		if [[ "|${AZURE_SIGNING_CLIENT_ID}|${AZURE_SIGNING_CLIENT_SECRET}|${AZURE_SIGNING_TENANT_ID}|${AZURE_SIGNING_ENDPOINT}|${AZURE_SIGNING_ACCOUNT}|${AZURE_SIGNING_CERTIFICATE_PROFILE}|" == *"||"* ]]; then \
			echo "Can't sign windows binaries as required configuration not set: AZURE_SIGNING_CLIENT_ID, AZURE_SIGNING_CLIENT_SECRET, AZURE_SIGNING_TENANT_ID, AZURE_SIGNING_ENDPOINT, AZURE_SIGNING_ACCOUNT, AZURE_SIGNING_CERTIFICATE_PROFILE"; \
			...
			java -jar bin/jsign-7.0.jar \
				--storetype TRUSTEDSIGNING \
				--keystore "${AZURE_SIGNING_ACCOUNT}" \
				--url "${AZURE_SIGNING_ENDPOINT}" \
				--alias "${AZURE_SIGNING_CERTIFICATE_PROFILE}" \
...
bin/jsign-7.0.jar:
	wget https://github.com/ebourg/jsign/releases/download/7.0/jsign-7.0.jar --output-document=bin/jsign-7.0.jar
