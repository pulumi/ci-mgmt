...
# Installs all necessary tools with mise
# And then ensures the environment is setup for this make session
mise_install:
	@mise install -q
	# send output to /dev/null because we don't need to see the entire env that mise exports
	@mise env -q  > /dev/null

# Build the provider and all SDKs and install ready for testing
build: mise_install provider build_sdks install_sdks
...
prepare_local_workspace: mise_install upstream
...
.PHONY: development only_build build generate generate_sdks build_sdks install_sdks mise_install
...
.make/generate_dotnet: mise_install bin/$(CODEGEN)
...
.make/generate_go: mise_install bin/$(CODEGEN)
...
.make/generate_java: mise_install bin/$(CODEGEN)
...
.make/generate_nodejs: mise_install bin/$(CODEGEN)
...
.make/generate_python: mise_install bin/$(CODEGEN)
...
.make/schema: bin/$(CODEGEN) mise_install .make/upstream
